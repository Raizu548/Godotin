...
func cargar_volumen_buses() -> void:
	for bus_indice in bus_indices.values():
		...
		bus_etiqueta[nombre_bus].text = texto_volumen
...
func cambiar_volumen(indice_bus: int, subir: bool) -> void:
	...
	comprobar_bus_master()
	if AudioServer.is_bus_mute(bus_indices.Master):
		silenciar_buses()
	else:
		activar_buses()
...
func comprobar_bus_master() -> void:
...
func silenciar_buses() -> void:
	for bus in bus_indices.values():
		AudioServer.set_bus_mute(bus,true)


func activar_buses() -> void:
	for bus in bus_indices.values():
